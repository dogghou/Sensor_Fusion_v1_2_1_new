...
def clouds_range_no_ground(int transform_x,int transform_y,np.int32_t[:,:] clouds, np.int16_t[:,:] range_ground_z):
    ...
    clouds_range_no_ground = np.zeros((clouds.shape[0],4), np.int32)
    for l in range(clouds.shape[0]): # 对clouds中的每一点
        if (clouds[l, 0] > -100) & (clouds[l, 0] < 4000) & (clouds[l, 1] > -4000) & (clouds[l, 1] < 3000) & (clouds[l, 2] < 1000):
            ...
            if (z_ground != 10000) & (z > (z_ground + 30)) & (z < (z_ground + 330)):
                clouds_range_no_ground[lp,:], lp = clouds[l,:], lp+1
...
def downsample_circle(np.int16_t[:,:] xel, np.int32_t[:] idex): # 依附于降采样下的循环操作
    ...
    cdef np.int16_t[:,:]  tmp, points_downsample
    cdef np.int32_t[:] tsum = np.zeros(4, np.int32)
    points_downsample = np.zeros((xel.shape[0], 4), np.int16) # mx, my, mz, sumI
    for i in range(idex.shape[0]):
        if i+1 < idex.shape[0]: tmp = xel[idex[i]:idex[i+1]]
        else: tmp = xel[idex[i]:]
        #print("tmp:", tmp)
        #print("tmp.shape[0]", tmp.shape[0])
...
def grid_circle(np.int16_t[:,:] voxel, np.int32_t[:] idex): # 依附于栅格化下的循环操作
    ...
    cdef np.int16_t[:,:] tmp, points_grid    
    points_grid = np.zeros((voxel.shape[0],7), np.int16) # 'sx','sy','x','y','z','zmax','I'
    for i in range(idex.shape[0]):
        ...
        #if (zmax-zmin >= 40) or (zmax >= 80):  # 则把tmp赋值给points_update
...
@cython.boundscheck(False)
@cython.wraparound(False)
def cluster_circle2(np.int16_t[:,:,:] points_slice, np.int16_t[:,:] grid_slice, np.int16_t[:,:] target_points, 
                    np.int32_t[:] idex, np.int32_t[:] idex2):
    ...
    cdef long i, j, lt2, lr2
    cdef np.int16_t[:,:] tmp2
    cdef np.int16_t[:] seed_grid2
    cdef np.int32_t[:] idex2_2, idex_round2
    for i in range(idex2.size): # 对每一个领域种子
        seed_grid2 = grid_slice[idex2[i]]  # 得到次一级种子栅格
        lt2 = 0
        for j in range(target_points.shape[0]): # 找到target_points中当前存入点的数目
            if target_points[j,2] == 0 and target_points[j,3] == 0:  # x, y 方向为零 
                break
            lt2 += 1
        tmp2 = points_slice[idex2[i]] # 该邻域栅格所包含的点云
        target_points[lt2:lt2+tmp2.shape[0]] = tmp2 # 把次级种子栅格的点云加入目标
        idex_round2, idex2_2, lr2 = np.zeros(8, np.int32), np.zeros(idex.shape[0], np.int32), 0 # 用于存放被选中的邻近栅格的的索引, lr2是idex_round2的索引

        for j in range(idex.size):
        ...
        idex = np.delete(np.ctypeslib.as_array(idex), np.ctypeslib.as_array(idex_round2[:lr2])) # 从idex中 删除 idex_round2的内容
        idex, target_points = cluster_circle2(points_slice, grid_slice, target_points, idex, idex2_2[:lr2])
    return idex, target_points

@cython.boundscheck(False)
@cython.wraparound(False)
def cluster_circle(np.int16_t[:,:,:] points_slice, np.int16_t[:,:] grid_slice, np.int32_t[:] idex, long cellsize=50):
    # 对栅格化点云的父级聚类
    # points_slice: sx[0], sy[1], x[2], y[3], z[4], zmax[5], I[6]
    # grid_slice: sx, sy, zmax
    cdef long i, lr, halfcell=25, ln = 0, pn   
    cdef int xc, yc, xmin, xmax, ymin, ymax, zmin, zmax, I, point_num
    cdef np.int16_t[:,:] target_points, tmp
    cdef np.int32_t[:,:] target_output
    cdef np.int16_t[:] seed_grid
    cdef np.int32_t[:] idex2, idex_round
    
    pn = long(idex.size)*50*5 # 以10为尺度进行降采样，以50为尺度进行删格化，一个删格内最多存在50个点云
    target_output = np.zeros((idex.size,9), np.int32)  # xc, yc, xmin, xmax, ymin, ymax, zmin, zmax, I   
    # t1 = time.time()
    while idex.size > 0:  # 只要栅格数量>0    
        target_points = np.zeros((pn,7), np.int16) # 初始化'sx','sy','x','y','z','zmax','I'...
        ...
        idex = np.delete(np.ctypeslib.as_array(idex), np.int16([0]))  # 索引中删除当前种子, 栅格数量减1
        idex_round, idex2, lr = np.zeros(8, np.int32), np.zeros(idex.size, np.int32), 0 # 用于存放被选中的邻近栅格的的索引, lr是idex_round的索引
        for i in range(idex.size):  # 对剩下的idex中的每一个索引
        ...
        # t1 = time.time()
        idex = np.delete(np.ctypeslib.as_array(idex), np.ctypeslib.as_array(idex_round[:lr]))  # 从原索引中删除已经形成种子的索引        
        idex, target_points= cluster_circle2(points_slice, grid_slice, target_points, idex, idex2[:lr])
        # print(time.time()-t1)        
        # target_points: sx[0], sy[1], x[2], y[3], z[4], zmax[5], I[6]
        xmin, ymin, zmin = np.copy(target_points[0,2]), np.copy(target_points[0,3]), np.copy(target_points[0,4])
        xmax, ymax, zmax = np.copy(target_points[0,2]), np.copy(target_points[0,3]), np.copy(target_points[0,4])
        I = np.copy(target_points[0,6])
        point_num = 1        
    ...
    while (lidar_data.shape[0]) > 0:
        ...
        for i in range(lidar_predict.shape[0]):
            ...
            #根据目标位置设定车辆的跟踪门大小
                ...
                if (distance_list.shape[0] == 1) or (distance_list[1] > 800):
                    dx_thresh = 500
                    dy_thresh = 500
...
@cython.boundscheck(False)
@cython.wraparound(False)
def heading(np.ndarray[np.float64_t, ndim=1] vxy, long thresh=500):
    # 根据速度向量求航向角
...
def tracked_targets_process(np.ndarray[np.int32_t, ndim=3] target_3Dlist, np.ndarray[np.int32_t, ndim=1] target_3Dlist_num, 
    ...
    for idex in range(target_3Dlist.shape[0]):
        ...
        if targetx.shape[0]>0:
            ...
            tmp_state[4] = long(0.8*last_lidar_state[idex,4] + 0.2*(tmp_state[2]-last_lidar_state[idex,2])/dt) # Vx
            tmp_state[5] = long(0.8*last_lidar_state[idex,5] + 0.2*(tmp_state[3]-last_lidar_state[idex,3])/dt) # Vy 
...
def lidar_movement(np.ndarray[np.int32_t, ndim=2] lidar_data, np.ndarray[np.int32_t, ndim=2] last_lidar_state, 
    ...
    for i in range(ll):
        ...
        if j < lidar_position.shape[2]: # 此时lidar_position未集齐足够position, 直接跟在后面赋值
            ...
            lidar_position[X, lidar_ID, j] = lidar_state[i, 2]
            lidar_position[Y, lidar_ID, j] = lidar_state[i, 3]
        else: 
            ...
            lidar_position[X, lidar_ID, j-1] = lidar_state[i,2] # xc
            lidar_position[Y, lidar_ID, j-1] = lidar_state[i,3] # yc
