...
def __init():
    ...
    is_amd64 = (platform.machine() == "x86_64")

    if is_win:
        if is_x86:
            _sdk = windll.LoadLibrary(basedir + "/CKCameraDLL.dll")
        else:
            a = basedir + "/CKCameraDLL_X64.dll"
            print(a)
        ...
        if is_amd64:
            _sdk = cdll.LoadLibrary(basedir + "/libCKCameraSDK_x64.so")
            CALLBACK_FUNC_TYPE = CFUNCTYPE
        else:
            _sdk = cdll.LoadLibrary(basedir + "/libCKCameraSDK_aarch64.so")
            CALLBACK_FUNC_TYPE = CFUNCTYPE
...
def CameraSaveParameterToFile(hCamera, fileName):
    ...
    ##
     ...
     #@param[in] hCamera The handle of the camera, obtained by the @link #CameraInit @endlink function
     #@param[in] *sFileNamUGSMT200C_Cfg_A.bine The full path to the parameter file.
     #@return @link #__CK_ERRCODE__ Status Code @endlink
