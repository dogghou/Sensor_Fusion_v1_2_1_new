...
def get_cluster(clouds_grid): 
    ...
    num_points = clouds_grid.shape[0]
    if num_points > 0:
        ...
        idex = np.arange(split.shape[0]).astype(np.int32)   # 一个idex为一个种子的索引
        split = np.append(split, num_points).astype(np.int32)
        targets = cluster_circle(points_slice, grid_slice, idex, split, num_points)  # xc, yc, xmin, xmax, ymin, ymax, zmin, zmax, I
    return targets
    """
    targets = np.zeros((0, 9), np.int32)
...
def read_lidar_process(lock, addr, Lidar_Data_Raw, flag, online=True, lidar_connect_state = False, frame=0):    
    ...
    while flag.value and time.time()-tick < 3 and p.is_running(): # 父进程要存在
        ...
        if is_next:  # 如果被判定为下一个循环
            ...
            # print("tick", tick)
            print("time_stamp:", (time_stamp[0]*16777216 + time_stamp[1]*65536 + time_stamp[2]*256 + time_stamp[3]))
